...
class PasswordGenerator {
    ...
    generatePassword(length: number, numbers: boolean, symbols: boolean): string {
        ...
        if (numbers) {
            if (!this.checkComplexityElement(password, this.NUMBERS)) {
                // TODO: change a random character to a number
                password.replace(Math.random() * (password.length - 1), this.NUMBERS.charAt(Math.random() * (this.NUMBERS.length - 1)));
            }
        } else if (symbols) {
            if (!this.checkComplexityElement(password, this.SYMBOLS)) {
                // TODO: change a random character to a number
                password.replace(Math.random() * (password.length - 1), this.SYMBOLS.charAt(Math.random() * (this.SYMBOLS.length - 1)));
            }
        }
        return password;
    }
